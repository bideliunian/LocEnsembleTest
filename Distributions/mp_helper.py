...
def mp_helper(ran, methods, space, metric, model, delta, n, nperm):
    ...
    for rep in range(lower, upper):
        ...
        x = generate_data_distribution_1d(n = n, m = 100, model=model, delta=0.)
        y = generate_data_distribution_1d(n = n, m = 100, model=model, delta=delta)
...
def multiprocess_helper(delta):
    result = mp_helper(ran=[0,5], methods=['angle', 'ball', 'energy', 'graph'], 
                    space='distribution', metric='wasserstein2', model='model1', 
                    delta=delta, r=1, n=40, nperm=99)
    return result
