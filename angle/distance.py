...
def pdist_distribution1d(X, metric='wasserstein2'):
    ...
    '''
    n = len(X)
    d_matrix = np.zeros([n, n])  # distance matric
...
def dist_distribution1d(X, Y, metric='wasserstein2'):
    ...
    '''
    m, n = len(X), len(Y)
    d_matrix = np.zeros([m, n])  # distance matric
...
def pdist_spd(X, metric='frobenius'):
    '''
    get distance_matrix for spd matrixes
...
def dist_spd(X, Y, metric='frobenius'):
    '''
    get distance_matrix for spd matrixes between two samples
